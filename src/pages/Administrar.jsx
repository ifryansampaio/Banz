...
const Administrar = () => {
  const [aba, setAba] = useState(0); // 0: Estoque, 1: Fechamento, 2: Lojas, 3: Funcionários, 4: Auditoria
    ...
    <div className="flex flex-col min-h-screen bg-gray-900 text-white">
      <div className="flex-1 w-full max-w-4xl mx-auto p-2 sm:p-4 md:p-6">
        ...
        <div className="flex border-b border-blue-800 mb-6">
          {['Estoque', 'Fechamento', 'Lojas', 'Funcionários', 'Auditoria'].map((nome, idx) => (
            <button
              key={nome}
              className={`px-4 py-2 font-bold focus:outline-none transition border-b-2 ${aba === idx ? 'border-blue-400 text-blue-300' : 'border-transparent text-gray-400 hover:text-blue-200'}`}
              onClick={() => setAba(idx)}
            >
              {nome}
            </button>
        ...
        <div className="bg-gray-800 p-2 sm:p-6 rounded-lg shadow-lg overflow-x-auto">
          ...
          {aba === 2 && (
            // ... Lojas (copiar bloco de lojas original)
            <div>
              <button className="bg-blue-700 hover:bg-blue-800 px-4 py-2 rounded font-bold text-white mb-6" onClick={exportarBackupFirestore}>Baixar Backup Completo</button>
              <h2 className="text-xl mb-4 font-semibold text-blue-200">Lojas</h2>
              <div className="mb-4 flex gap-2">
                <input type="text" placeholder="Nome da Loja" className="p-3 border text-black rounded w-full focus:outline-none focus:ring-2 focus:ring-blue-400" value={novaLoja.nome} onChange={(e) => setNovaLoja({ nome: e.target.value })} />
                  ...
                  <li key={loja.id} className="py-3">
                    {editandoLoja && editandoLoja.id === loja.id ? (
                      <div className="bg-gray-900 p-4 rounded-lg flex flex-col gap-2 shadow-lg">
                        <input type="text" className="text-black p-1 rounded" value={editandoLoja.nome} onChange={e => setEditandoLoja({ ...editandoLoja, nome: e.target.value })} />
          ...
          {aba === 4 && (
            // ... Auditoria (copiar bloco de auditoria original)
            <div>
